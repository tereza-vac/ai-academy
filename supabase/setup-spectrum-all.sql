-- ===========================================================================
-- AI Academy — Spektrum modelů one-shot bootstrap
-- ===========================================================================
-- Run ONCE in Supabase Studio -> SQL Editor (project iusyhjcckowaqlykbahe).
--
-- Fixes: Could not find the table public.llm_models in the schema cache
--
-- After SQL succeeds: deploy models-ingest, optional POST sync, reload app.
-- ===========================================================================

-- LLM model catalog ("Spektrum modelĹŻ")
...
-- Curated LLM catalog entries with rich editorial content.
-- models-ingest updates pricing/context for matching slugs but preserves these fields.
...
(
  ...
  'Flagship multimodal model from OpenAI â€” fast, capable, widely deployed.',
...
(
  ...
  E'## Overview\n\nGPT-4o mini trades a small quality gap for **much lower cost** â€” ideal for classification, routing, and chat at scale.',
...
(
  ...
  'Balanced Claude model â€” strong coding and analysis with a large context window.',
...
(
  ...
  E'## Overview\n\nOpus is Anthropic''s **most capable** tier â€” use when quality matters more than unit economics.',
...
(
  ...
  E'## Overview\n\nLlama 3.3 70B is **open weights** (custom license) â€” self-host, fine-tune, or run via hosted APIs.',
...
(
  ...
  'European flagship API model â€” strong multilingual and coding.',
...
(
  ...
  'Sparse MoE open model â€” efficient inference for its quality class.',
  E'## Overview\n\nMixtral uses a **Mixture-of-Experts** architecture â€” only a subset of parameters activate per token.',
...
(
  ...
  'High-performing open model family from Alibaba Cloud â€” strong in math and code.',
...
(
  ...
  E'## Overview\n\nDeepSeek V3 disrupted pricing expectations â€” strong reasoning at **fraction of frontier API cost**.',
...
(
  ...
  'Small language model optimized for reasoning â€” runs on modest hardware.',
...
(
  ...
  E'## Overview\n\nCommand R+ is built for **grounded generation** â€” cite sources, connect to search connectors.',
  array['Enterprise RAG','Search augmentation','Tool calling'],
  array['RAG-first design','Multilingual retrieval'],
  array['Less general â€śchat personalityâ€ť than consumer chatbots'],
...
(
  ...
  E'## Overview\n\nGrok emphasizes **real-time** knowledge and a distinct tone â€” niche for social/news use cases.',
...
(
  ...
  'Databricks MoE model â€” strong open benchmark scores at launch.',
  E'## Overview\n\nDBRX is a **training showcase** for Databricks Mosaic â€” less common in greenfield apps today.',
...
(
  ...
  'Open image generation model family â€” not an LLM but often grouped in â€śgenerative modelsâ€ť.',
  E'## Overview\n\nIncluded for **multimodal literacy** â€” text-to-image, not chat completion.',
...
(
  ...
  'Runtime to pull and run open models locally â€” not a single model but a deployment path.',
  ...
  array['Not a hosted SLA â€” ops are on you'],
...
-- 03:42 UTC daily â€” off-peak, distinct from radar (:17).
