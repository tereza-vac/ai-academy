/**
 * ChatWidget — professional floating AI Tutor chat bubble.
 *
 * Uses createPortal into document.body to bypass any overflow/stacking issues.
 * Only uses Tailwind tokens confirmed in tailwind.config.ts.
 */
import {
  useCallback,
  useEffect,
...
const WELCOME = {
  ...
  cs: "Ahoj 👋 Jsem tvůj **AI Tutor**. Zeptej se mě na cokoliv o umělé inteligenci nebo konceptech z Akademie.",
};
  ...
  return (
    <div className="flex flex-col items-center gap-5 px-4 py-6 text-center h-full justify-center">
      <div className="relative">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/15 shadow-elevation-sm">
          <Sparkles className="h-8 w-8 text-primary" />
        </div>
        <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-success border-2 border-surface-elevated text-[9px] text-success-foreground font-bold">
          ✓
        </span>
      </div>
...
function ChatPanel({
  ...
  const welcomeMsg = conceptLabel
    ? locale === "cs"
      ? `Ahoj 👋 Pojďme probrat **${conceptLabel}**. Na co se chceš zeptat?`
  ...
  // Handle pending messages from the store
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden rounded-2xl">
      ...
      {/* ── Header ─────────────────────────────────────────────────── */}
      <div className="shrink-0 bg-primary px-4 py-4">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-[11px] text-white/70">
                {isStreaming
                  ...
                  : (conceptLabel ?? (locale === "cs" ? "Online" : "Online"))}
              </p>
            </div>
          </div>
          <div className="flex items-center">
            <button type="button" title={locale === "cs" ? "Nová konverzace" : "New conversation"} onClick={resetSession}
              className="flex h-7 w-7 items-center justify-center rounded-lg text-white/60 hover:bg-white/15 hover:text-white transition-colors">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto scrollbar-thin bg-surface-base"
      >
        {!hasContent
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          "shadow-[0_8px_24px_rgba(0,0,0,0.2),0_2px_8px_rgba(0,0,0,0.15)]",
      ...
      >
        ...
        {!isOpen && (
          <span className="absolute inset-0 rounded-full bg-primary/30 animate-ping opacity-0 [animation-play-state:paused]" />
        )}

        {/* Icon: MessageSquare when closed, X when open */}
        <span className={cn(
